  ...
  if(random(4) == 1) {
    switch (random(10)) {
      case 0 ... 5: //50% chance of getting coins
        ...
        createFoundLootMessage(0,2);
        break;
      case 6 ... 8: //30% chance of getting a heart
        player.hlth++;
        createFoundLootMessage(3,1);
        break;
      default: //20% chance of getting a gem
        player.gems++;
        createFoundLootMessage(1,1);
        break;
    }
  }
}
    ...
    if(en[x].exist) {
      ...
      switch (en[x].type) {
        ...
        case 3: //Freezer
          ...
          }if(collideWithPlayer(attRadius) and !alreadyExists(1) and player.frozen == 0) createProjectile(en[x].x - 8, en[x].y - 8, 0, 0, 1);
          break;
        case 4: //Magmar
