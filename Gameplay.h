...
//Loot types: 0 = coins, 1 = gems, 2 = keys, 3 = health, 4 = dur+ (increase swDur and shDur), 5 = reg+ (decrease rgDur), 6 = hlth+ (increase max amount of health), 7 = canSwim:true, 8 = shatterIce:true, 9 = teleport
    ...
    if(chestLoot[id][0] == Map and solidCollisionWith(playerRect(),11) and !opened[id]) {
      ...
      switch (chestLoot[id][1]) {
        ...
        case 2:
          ...
          break;
      }createFoundLootMessage(chestLoot[id][1],chestLoot[id][2]);
      break;
    }
  }
}
...
void drawHUD() {
  ...
  if(player.coins < 10) {arduboy.print(F("x00"));arduboy.print(player.coins);}
  else if(player.coins < 100) {arduboy.print(F("x0"));arduboy.print(player.coins);}
  else {arduboy.print('x');arduboy.print(player.coins);}
  arduboy.setCursor(107,47);
  if(player.keys < 10) {arduboy.print(F("x0"));arduboy.print(player.keys);}
  else {arduboy.print('x');arduboy.print(player.keys);}
  arduboy.setCursor(107,57);
  if(player.gems < 10) {arduboy.print(F("x0"));arduboy.print(player.gems);}
  else {arduboy.print('x');arduboy.print(player.gems);}
}
void drawMap() {
  for(uint8_t y; y < 96; y++){
    char blck = getBlock(y%12,y/12);
